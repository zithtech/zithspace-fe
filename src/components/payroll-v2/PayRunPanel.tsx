...
export default function PayRunPanel() {
  const {
    ...
    canFinalizePayrollRun, canPayPayrollRun, canGeneratePayrollPayslips, canDeletePayrollRun, canManagePayroll } = usePermission();
    ...
    <div className="pvr">
      ...
      <div className="pvr-table-wrap" style={{ position: 'relative' }}>
        ...
        {runs.length === 0 && !loading
          ...
          : <Table rowKey="id" size="small" className="pvr-table" loading={false} columns={runColumns} dataSource={runs} pagination={false} onRow={(r) => ({ onClick: () => openDetail(r), style: { cursor: 'pointer' } })} scroll={{ x: 'max-content' }} />}
      </div>
