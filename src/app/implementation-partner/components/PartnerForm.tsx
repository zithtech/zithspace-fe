...
export default function PartnerForm({ id, mode }: PartnerFormProps) {
  ...
  const fetchPartnerData = async (partnerId: string) => {
    ...
    try {
      ...
      message.error("Failed to load partner information");
    } finally {
      setFetching(false);
    }
  };
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      message.error("Failed to save implementation partner");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={{ padding: "24px", background: "white", minHeight: "calc(100vh - 64px)", paddingBottom: "100px" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          {isView && (
            <Button type="primary" icon={<EditOutlined />} onClick={() => router.push(`/implementation-partner/edit/${id}`)}>
              Edit Partner
            </Button>
          )}
        </div>
            ...
            <Row gutter={24}>
              <Col span={12}>
                <Form.Item name="website" label="Website URL">
                  <Input placeholder="e.g. www.partner.com" />
                </Form.Item>
              </Col>
              <Col span={12}>
                <Form.Item name="companyEmail" label="Company Email">
                  <Input placeholder="e.g. contact@partner.com" />
                </Form.Item>
              </Col>
            </Row>
            <Row gutter={24}>
