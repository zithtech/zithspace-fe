  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} onClick={() => onClick(deal)}>
      <Card
        ...
        hoverable
        bordered
        style={{ borderColor: "var(--border-slate-100)", borderRadius: "8px", background: 'var(--bg-pure-white)' }}
        styles={{ body: { padding: "14px 16px" } }}
      >
        <Space direction="vertical" size={10} style={{ width: "100%" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <Text strong style={{ fontSize: "13px", color: "var(--text-slate-900)", display: "block", marginBottom: "2px", lineHeight: 1.3 }}>
              {deal.title}
            </Text>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderTop: "1px solid var(--border-slate-100)", paddingTop: "8px" }}>
            <Space size={4}>
              <ClockCircleOutlined style={{ color: isOverdue ? "var(--text-leave)" : "var(--text-slate-400)", fontSize: "12px" }} />
              <Text style={{ fontSize: '11px', color: isOverdue ? 'var(--text-leave)' : 'var(--text-slate-500)' }}>
                {formattedDate}
              </Text>
            </Space>
            ...
            <Tooltip title={deal.assignedTo ? `${deal.assignedTo.first_name} ${deal.assignedTo.last_name}` : "Unassigned"}>
              {deal.assignedTo ? (
                <Avatar size={20} style={{ backgroundColor: "var(--premium-blue)" }} icon={<UserOutlined />} />
              ) : (
