...
import { Table, Tag, Typography, Space, Row, Col, Select, Avatar, Tooltip, Button, DatePicker, Collapse } from "antd";
import {
  ClockCircleOutlined,
  HistoryOutlined,
  ReloadOutlined,
...
export const PerformanceTracker: React.FC<PerformanceTrackerProps> = ({ refreshKey }) => {
  ...
  // Quick month range (from month → to month). Drives the date range.
  ...
  }, [filters.userIds, filters.projectId, filters.dateRange, refreshKey]);
  ...
  const hasActiveFilters = !!(filters.userIds.length || filters.projectId || monthRange);
  ...
  const guideItems = useMemo(() => {
    ...
    const base = [
      ...
      ...legend.weekend.map((w) => ({ label: w.label, range: `${w.day} · if tracked` })),
    ];
    ...
    <div style={{ display: "flex", flexDirection: "column", flex: 1 }}>
      ...
      <div className="perf-filterbar">
          <div className="mtt-team-filters">
            <SearchableDropdown
              ...
              onChange={(v) => setFilters((f) => ({ ...f, userIds: (v as string[]) || [] }))}
            ...
            <RangePicker
              picker="month"
              className="mtt-team-filters__month"
            ...
            <Tooltip title="Refresh data">
              <Button onClick={fetchPerformance} icon={<ReloadOutlined />} loading={loading} className="mtt-tracker-card__action mtt-team-card__refresh" size="small" />
            </Tooltip>
          </div>
      </div>
      ...
      {insight && (
        <div className="perf-insight">
          ...
          <span className="perf-insight__text">{insight}</span>
        </div>
      )}
            ...
            locale={{
              emptyText: (
                <div className="mtt-tracker-card__empty">
                  <div className="mtt-tracker-card__empty-icon"><TrophyOutlined /></div>
                  <div className="mtt-tracker-card__empty-title">No performance data in this range</div>
                  <div className="mtt-tracker-card__empty-sub">Pick a project, members and a date range to see results.</div>
                </div>
              ),
            }}
          />
      ...
      <style jsx global>{`
        ...
        .mtt-team-filters__month { width: 240px !important; min-width: 0 !important; }
        .mtt-team-filters__range { width: 208px !important; }
        .mtt-team-filters .ant-picker { border-radius: 6px; }
        /* Insight banner */
        .perf-insight {
          display: flex;
          align-items: center;
