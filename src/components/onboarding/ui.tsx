    ...
    <div className="onb-sprint-header-v2">
      <div className="onb-sprint-row1">
        <div className="onb-sprint-title-block">
          <div className="onb-sprint-dot" style={{ background: '#3B82F6' }} />
          <h2 className="onb-sprint-title">{title}</h2>
          <div className="onb-sprint-tags">
            <Tag className="onb-sprint-tag onb-sprint-tag-active" bordered={false} style={{ color: statusColor, borderColor: statusBorder }}>{statusText}</Tag>
          </div>
        </div>
      ...
      <div className="onb-sprint-row3">
        <div className="onb-sprint-progress-bar">
          <div className="onb-sprint-progress-fill" style={{ width: progressPct || '100%', background: '#3B82F6' }} />
        </div>
        <div className="onb-sprint-progress-pct">{progressPct || '100%'}</div>
      </div>
    </div>
  );
}
...
export function OnbStyles() {
  return (
    <style jsx global>{`
      /* Sprint Header (Stats) */
      .onb-sprint-header-v2 { display: flex; flex-direction: column; gap: 2px; padding: 4px 20px 6px; background: var(--bg-pure-white); border-bottom: 1px solid var(--border-slate-200); margin-bottom: 0px; flex-shrink: 0; }
      .onb-sprint-row1 { display: flex; align-items: center; justify-content: space-between; gap: 8px; flex-wrap: wrap; margin-bottom: 2px; }
      .onb-sprint-title-block { display: flex; align-items: center; gap: 6px; min-width: 0; flex: 1 1 auto; }
      .onb-sprint-dot { width: 6px; height: 6px; border-radius: 50%; flex-shrink: 0; }
      .onb-sprint-title { font-size: 13px !important; font-weight: 800 !important; color: var(--text-slate-900) !important; letter-spacing: -0.01em; margin: 0; }
      .onb-sprint-tags { display: inline-flex; align-items: center; gap: 4px; flex-shrink: 0; }
      .onb-sprint-tag { display: inline-flex; align-items: center; height: 16px; padding: 0 4px; font-size: 9px; font-weight: 800; letter-spacing: 0.04em; border-radius: 4px; border: 1px solid transparent; text-transform: uppercase; line-height: 1; }
      .onb-sprint-tag-active { background: transparent; color: #34d399; border-color: rgba(16, 185, 129, 0.32); }
      .onb-sprint-row2 { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; padding-left: 14px; margin-bottom: 4px; }
      .onb-sprint-meta { display: inline-flex; align-items: center; gap: 4px; font-size: 11px; font-weight: 600; color: var(--text-slate-500); letter-spacing: -0.005em; }
      .onb-sprint-meta b { color: var(--text-slate-900); font-weight: 800; }
      .onb-sprint-row3 { display: flex; align-items: center; gap: 10px; padding-left: 14px; }
      .onb-sprint-progress-bar { flex: 1 1 auto; position: relative; height: 5px; background: var(--bg-slate-100); border-radius: 999px; overflow: hidden; min-width: 60px; }
      .onb-sprint-progress-fill { position: absolute; inset: 0; border-radius: 999px; transition: width 0.4s ease; }
      .onb-sprint-progress-pct { flex-shrink: 0; font-size: 11px; font-weight: 800; color: var(--text-slate-900); font-variant-numeric: tabular-nums; min-width: 32px; }
