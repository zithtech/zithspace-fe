...
export interface QaSubmissionsFiltersState {
...
interface QaSubmissionsFiltersProps {
  filters: QaSubmissionsFiltersState;
  onFilterChange: (key: keyof QaSubmissionsFiltersState, value: any) => void;
  onReset?: () => void;
  projectOptions: { value: string; label: string }[];
  scopeOptions: { value: string; label: string; description?: string }[];
  ownerOptions: { value: string; label: string; avatarUrl?: string | null }[];
  statusOptions: { value: string; label: string }[];
  recommendationOptions: { value: string; label: string }[];
}

const QaSubmissionsFilters: React.FC<QaSubmissionsFiltersProps> = ({
  filters,
  onFilterChange,
  onReset,
  projectOptions,
  ...
  const activeCount =
    (filters.projectFilter ? 1 : 0) +
    (filters.scopeFilter ? 1 : 0) +
    (filters.ownerFilter ? 1 : 0) +
    (filters.statusFilter ? 1 : 0) +
    (filters.recommendationFilter ? 1 : 0) +
    (filters.dateRange?.[0] ? 1 : 0);

  return (
    <div className="tf-panel">
      <style dangerouslySetInnerHTML={{ __html: QA_FILTERS_CSS }} />
      ...
      <div className="tf-body">
        <div className="tf-row">
          ...
          <TicketFilterPill
            ...
            onChange={(val) => onFilterChange("projectFilter", val)}
            placeholder="Any"
            itemNoun="projects"
            width={200}
            multiple={false}
        ...
        <div className="tf-row">
          <div className="tf-row-label">
            <AimOutlined className="tf-row-icon" />
            <span>Scope</span>
          </div>
          <TicketFilterPill
            label="Scope"
            ...
            onChange={(val) => onFilterChange("scopeFilter", val)}
            placeholder="Any"
            itemNoun="scopes"
            width={200}
            multiple={false}
        ...
        <div className="tf-row">
          ...
          <TicketFilterPill
            ...
            onChange={(val) => onFilterChange("recommendationFilter", val)}
            placeholder="Any"
      ...
      <div className="tf-foot">
        <Text className="tf-foot-hint">
          {activeCount === 0
            ? "No filters applied — showing all submissions."
...
export default QaSubmissionsFilters;

const QA_FILTERS_CSS = `
