...
interface BucketFiltersProps {
  ...
  onFilterChange: (key: keyof BucketFiltersState, value: any) => void;
  onReset?: () => void;
...
/**
 * The Filters popover for Buckets — the same panel shape the Ticket List and
 * the QA lists use. These four used to live in the left rail; the rail is gone.
 */
const BucketFilters: React.FC<BucketFiltersProps> = ({
  filters,
  onFilterChange,
  onReset,
  visibilityOptions,
  ...
  const activeCount =
    (filters.visibility && filters.visibility !== "all" ? 1 : 0) +
    (filters.owner ? 1 : 0) +
  ...
  return (
    <div className="tf-panel">
      <style dangerouslySetInnerHTML={{ __html: BUCKET_FILTERS_CSS }} />
      ...
      <div className="tf-body">
        <div className="tf-row">
          <div className="tf-row-label">
            <EyeOutlined className="tf-row-icon" />
            <span>Visibility</span>
          </div>
          <TicketFilterPill
            label="Visibility"
            value={filters.visibility && filters.visibility !== "all" ? filters.visibility : ""}
            options={visibilityOptions}
            onChange={(val) => onFilterChange("visibility", val || "all")}
            placeholder="Any"
            itemNoun="options"
            width={200}
            multiple={false}
        ...
        <div className="tf-row">
          ...
          <TicketFilterPill
            ...
            onChange={(val) => onFilterChange("owner", val || null)}
            placeholder="Anyone"
            itemNoun="owners"
            width={240}
            showAvatar
        ...
        <div className="tf-row">
          ...
          <TicketFilterPill
            label="Bucket size"
            value={filters.size && filters.size !== "all" ? filters.size : ""}
            options={sizeOptions}
            onChange={(val) => onFilterChange("size", val || "all")}
            placeholder="Any"
            itemNoun="ranges"
            width={220}
            multiple={false}
      ...
      <div className="tf-foot">
        <Text className="tf-foot-hint">
          {activeCount === 0
            ? "No filters applied — showing every bucket."
...
export default BucketFilters;

const BUCKET_FILTERS_CSS = `
