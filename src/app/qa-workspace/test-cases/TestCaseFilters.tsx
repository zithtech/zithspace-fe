...
interface TestCaseFiltersProps {
  ...
  onFilterChange: (key: keyof TestCaseFiltersState, value: any) => void;
  onReset?: () => void;
  moduleOptions: { value: string; label: string }[];
  statusOptions: { value: string; label: string }[];
  automationOptions: { value: string; label: string }[];
  ownerOptions: { value: string; label: string }[];
}

/**
 * The Filters popover for the Test Cases list — the same panel shape the
 * Ticket List and the other QA lists use, so the lists all filter alike.
 */
const TestCaseFilters: React.FC<TestCaseFiltersProps> = ({
  filters,
  onFilterChange,
  onReset,
  moduleOptions,
  ...
  return (
    <div className="tf-panel">
      <style dangerouslySetInnerHTML={{ __html: CASE_FILTERS_CSS }} />
      ...
      <div className="tf-foot">
        <Text className="tf-foot-hint">
          {activeCount === 0
            ? "No filters applied — showing all cases."
