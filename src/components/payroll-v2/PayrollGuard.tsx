...
import { getPayrollNavItem, canAccessPayrollItem } from './navItems';

// Route-level permission gate for a Payroll 2.0 sub-page. Pass the nav item key;
...
export default function PayrollGuard({
  ...
  const item = getPayrollNavItem(itemKey);

  const allowed = item ? canAccessPayrollItem(perms, item) : false;
