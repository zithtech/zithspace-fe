...
import { ClockCircleOutlined, ProjectOutlined, CheckCircleOutlined } from '@ant-design/icons';
...
export default function UpdateCard({ update, onClick }: UpdateCardProps) {
  ...
  const getStatusConfig = (status: string) => {
    const configs: Record<string, { icon: string; color: string; label: string }> = {
      ...
      pushed_to_production: { icon: '🎉', color: 'purple', label: 'Production' },
    };
  ...
  return (
    ...
    >
        ...
        <Space size={8} align="center" style={{ width: '100%' }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <Space size={6} wrap>
              ...
              <Tag icon={<ClockCircleOutlined />} color="blue" style={{ fontSize: 11, margin: 0 }}>
                {formatHours(totalHours)}
              </Tag>
            </Space>
          </div>
        </Space>
        <Text type="secondary" style={{ fontSize: 11, display: 'block', marginTop: 4, marginLeft: 44 }}>
      ...
      <div style={{ marginBottom: 10 }}>
        {projectCount === 1 ? (
          <div style={{ padding: 8, backgroundColor: '#fafafa', borderRadius: 4, border: '1px solid #e8e8e8' }}>
            <Text style={{ fontSize: 12 }}>
      ...
      <div style={{ marginBottom: 10, padding: 10, backgroundColor: '#fafafa', borderRadius: 4, border: '1px solid #e8e8e8' }}>
        <Text strong style={{ fontSize: 11, display: 'block', marginBottom: 8, color: '#595959' }}>
          ⏰ Time Tracking
        </Text>
        <Space direction="vertical" size={4} style={{ width: '100%' }}>
          <Row justify="space-between">
            <Text style={{ fontSize: 10, color: '#8c8c8c' }}>Start:</Text>
            <Text strong style={{ fontSize: 11 }}>{formatTime(earliestStart)}</Text>
          </Row>
          <Row justify="space-between">
            <Text style={{ fontSize: 10, color: '#8c8c8c' }}>End:</Text>
            <Text strong style={{ fontSize: 11 }}>{formatTime(latestEnd)}</Text>
          </Row>
          <div style={{ borderTop: '1px solid #e8e8e8', margin: '4px 0' }} />
          <Row justify="space-between">
            <Text strong style={{ fontSize: 10 }}>Total:</Text>
            <Text strong style={{ fontSize: 12 }}>{formatHours(totalHours)}</Text>
          </Row>
        </Space>
      </div>
      ...
      {Object.keys(statusSummary).length > 0 && (
        <div style={{ marginBottom: 10, padding: 10, backgroundColor: '#fafafa', borderRadius: 4, border: '1px solid #e8e8e8' }}>
          ...
          <Space direction="vertical" size={3} style={{ width: '100%' }}>
            ...
            {['pushed_to_production', 'pushed_to_staging', 'in_testing', 'dev_complete', 'in_progress', 'pending']
                ...
                return (
                  <Row key={status} justify="space-between" align="middle">
                    <Space size={4}>
                      ...
                      <Text style={{ fontSize: 10, color: '#8c8c8c' }}>{config.label}</Text>
                    </Space>
