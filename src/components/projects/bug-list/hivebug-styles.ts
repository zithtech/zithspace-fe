...
.hb-section-grow { flex: 1; overflow-y: auto; padding-bottom: 12px; }
.hb-section-title {
...
.hb-header-tools { display: flex; align-items: center; gap: 8px; }
...
.hb-filterbar-close:hover { background: var(--hb-bg-hover); color: var(--hb-text); }
.hb-filter-reset { padding: 4px 10px; font-size: 12px; }
...
/* AntD Select polish inside the filter bar */
...
.hb-filterbar :where(.ant-select .ant-select-selection-placeholder) {
  color: var(--hb-text-muted) !important;
}
.hb-dark .hb-filterbar :where(.ant-select .ant-select-selection-item) {
  color: var(--hb-text);
}
...
.hb-table { width: 100%; border-collapse: separate; border-spacing: 0; }
.hb-table thead th {
...
@media (max-width: 768px) {
  ...
  .hb-content { overflow: visible; padding: 8px 10px 12px; }
  .hb-header {
    padding: 10px 12px;
    gap: 10px;
  }
  ...
  .hb-table-wrapper { overflow-x: auto; -webkit-overflow-scrolling: touch; }
  .hb-table { min-width: 880px; }
  .hb-pagination {
    flex-direction: column;
    align-items: stretch;
    gap: 8px;
