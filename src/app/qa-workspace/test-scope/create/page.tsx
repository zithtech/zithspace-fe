...
export default function CreateScopePage() {
  ...
  /**
   * Prefill QA Owner with the signed-in user and Reviewer with their reporting
   * manager, once the member list is available. Only fills empty fields, and
   * never marks the form dirty — the user still has to save deliberately.
   */
  useEffect(() => {
    if (defaultsAppliedRef.current) return;
    if (!user?.id || usersList.length === 0) return;

    const nameOf = (u: any) =>
      u?.name || u?.full_name || `${u?.first_name ?? ''} ${u?.last_name ?? ''}`.trim();

    const me = usersList.find(u => String(u.id) === String(user.id));
    const myName = nameOf(me) || user.name || '';

    // `reportsTo` comes back as an object on the member record and as an id on the auth user.
    const managerId = typeof me?.reportsTo === 'object' && me?.reportsTo
      ? me.reportsTo.id
      : (me?.reportsTo || (user as any).reportsTo || null);
    const managerName =
      (typeof me?.reportsTo === 'object' && me?.reportsTo?.name) ||
      nameOf(usersList.find(u => String(u.id) === String(managerId))) ||
      '';

    if (!myName && !managerName) return;

    defaultsAppliedRef.current = true;
    skipDirtyRef.current = true;
    setFormData((prev: any) => ({
      ...prev,
      qa_owner: prev.qa_owner || myName,
      details: {
        ...prev.details,
        reviewer: prev.details.reviewer || managerName,
      },
    }));
  }, [user, usersList]);
  ...
  }, [isLoading, canCreateScope]);

  useEffect(() => {
    if (!isLoading && canCreateScope) {
      fetchSprintsSearch("");
  ...
  if (!canCreateScope) return <div className="p-10 text-center text-slate-500">Unauthorized</div>;
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div className="flex items-start gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <h1 className="ts-title mt-1.5">{formData.name?.trim() || 'Create Test Scope'}</h1>
                </div>
              </div>
            ...
            <main className="flex-1 min-w-0 flex flex-col gap-5 pb-24">
              ...
              <div className="flex items-center justify-between gap-4 flex-wrap px-1">
                <p className="text-[12px] m-0" style={{ color: 'var(--ts-text-3)' }}>
                  {isDirty ? 'Changes are kept in this tab until you save.' : 'Nothing to save yet.'}
                </p>
                <div className="flex items-center gap-2.5">
                  <Button onClick={() => router.back()}>Cancel</Button>
                  <Button type="primary" icon={<Save size={15} />} onClick={handleSave} loading={submitting}>
