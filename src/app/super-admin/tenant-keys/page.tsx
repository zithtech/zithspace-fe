  ...
  const columns = [
    {
      ...
      render: (text: string) => (
        <div className="flex items-center gap-2">
          ...
          <Text strong className="text-gray-800 text-[13px]">{text}</Text>
        </div>
      ),
    },
    {
      ...
      key: "subdomain",
      render: (text: string) => (
        <Tag color="blue" className="px-3 py-1 rounded-full font-medium border-blue-200 bg-blue-50 text-blue-700">
          {text}
        </Tag>
      ),
    },
      ...
      render: (_: any, record: Tenant) => (
        record.web_inquiry_secret_key ? (
          ...
          <div className="flex items-center gap-1.5 text-rose-500 font-medium">
            <WarningFilled className="text-rose-500" />
    ...
    },
    {
      ...
      render: (key: string | null) => 
        key ? (
          <div className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-1.5 inline-flex items-center group transition-colors hover:bg-gray-100 hover:border-gray-300">
            <Text copyable={{ text: key }} className="text-gray-600 font-mono text-[13px]">
              {key}
            </Text>
          </div>
        ) : (
          <Text className="text-gray-400 italic text-[13px]">No key assigned</Text>
        ),
    },
    {
      title: "Action",
      ...
      render: (_: any, record: Tenant) => (
        <Button
          ...
          className={`h-8 px-3 text-[13px] ${record.web_inquiry_secret_key ? "" : "bg-gradient-to-r from-blue-600 to-indigo-600 border-none shadow hover:shadow-md transition-all"}`}
          icon={record.web_inquiry_secret_key ? <SafetyCertificateOutlined className="text-emerald-500" /> : <ApiOutlined />}
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 sm:p-8">
      <div className="max-w-7xl mx-auto">
        ...
        >
          {/* Header Section */}
          <div className="bg-white border-b border-gray-100 px-4 py-4 sm:px-6 sm:py-5">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-lg bg-indigo-50 flex items-center justify-center border border-indigo-100">
                  <KeyOutlined className="text-lg text-indigo-600" />
                </div>
                <div>
                  <Title level={5} className="!m-0 text-gray-900 font-bold tracking-tight">
              ...
              <Space size="small">
                <Tooltip title="Refresh Data">
                  <Button
                    ...
                    loading={loading}
                    className="h-8 w-8 px-0 flex items-center justify-center border-gray-200 text-gray-600 hover:text-indigo-600 hover:border-indigo-600"
                  />
                </Tooltip>
                <Button
                  type="primary"
          ...
          <div className="p-0">
            <Table
              ...
              pagination={{ 
                pageSize: 15,
                className: "px-4 py-3 border-t border-gray-100 m-0",
