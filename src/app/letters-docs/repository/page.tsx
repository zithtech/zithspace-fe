...
import { Table, Button, Dropdown, Tooltip, Select, Drawer } from 'antd';
...
export default function DocumentRepositoryPage() {
      ...
      render: (_: any, doc: GeneratedDocument) => (
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '6px' }}>
          <button
            onClick={(e) => {
              e.stopPropagation();
              router.push(`/letters-docs/generate?editId=${doc.id}`);
            }}
            style={{
              padding: '4px 8px',
              borderRadius: '4px',
          ...
          <button
            ...
            style={{
              ...
              fontSize: '11px',
              fontWeight: 600,
              cursor: downloadingDocs[doc.id] === 'pdf' ? 'not-allowed' : 'pointer',
              display: 'inline-flex',
              alignItems: 'center',
              gap: '4px',
              opacity: downloadingDocs[doc.id] === 'pdf' ? 0.7 : 1,
            }}
            title="Download PDF"
          >
            {downloadingDocs[doc.id] === 'pdf' ? (
          ...
          <button
            ...
            style={{
              ...
              fontSize: '11px',
              fontWeight: 600,
              cursor: downloadingDocs[doc.id] === 'docx' ? 'not-allowed' : 'pointer',
              display: 'inline-flex',
              alignItems: 'center',
              gap: '4px',
              opacity: downloadingDocs[doc.id] === 'docx' ? 0.7 : 1,
            }}
            title="Download DOCX"
          >
            {downloadingDocs[doc.id] === 'docx' ? (
              <Loader2 size={12} className="animate-spin" />
            ) : (
              <Download size={12} />
            )}
  ...
  return (
    <div className="doc-repository-container" style={{ display: 'flex', flexDirection: 'column', flex: 1, minWidth: 0, minHeight: 0 }}>
      ...
      <div style={{ padding: '24px 28px 40px', flex: 1, overflow: 'hidden', minHeight: 0, display: 'flex', flexDirection: 'column' }}>
                ...
                <div className="pc-foot">
                  <div className="pc-foot-row">
                    <span className="pc-foot-item">
                      ...
                      <span className="pc-foot-val">{doc.generatedBy ? doc.generatedBy.name : 'System'}</span>
                    </span>
      ...
      {deleteDocId && (
        <ConfirmDialog
          open={true}
          title="Delete Generated Document Record?"
