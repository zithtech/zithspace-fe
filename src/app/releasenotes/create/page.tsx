...
//   // ✅ Fetch environments using getEnviroments
//   useEffect(() => {
//     const fetchEnvironments = async () => {
//       try {
//         setLoadingEnvironments(true);
//         const response = await enviromentService.getEnviroments(1, 100);
...
//           setEnvironments(response.data);
//         } else {
//           console.log("Unexpected environments response format:", response);
//           setEnvironments([]);
//         }
//       } catch (error) {
...
//           : undefined,
//         repositories: releaseData.repositories?.length
//           ? [{ repository: releaseData.repositories[0] }]
//           : undefined,
//         pullRequests: releaseData.pullRequests?.length
//           ? [{ pr: releaseData.pullRequests[0] }]
//           : undefined,
//       });
...
//   // ✅ Fetch existing versions when project is selected
//   useEffect(() => {
...
//   // ✅ Handle version input change and check for duplicates
...
//   // ✅ Fetch ALL tickets when project is selected
...
//         console.log(
//           `✅ Loaded ${ticketsData.length} total tickets, showing latest 10`,
//         );
//       } else {
//         setAllTickets([]);
//         setDisplayTickets([]);
...
//   // ✅ Validate version before publish
...
//       const repositories = values.repositories?.[0]?.repository
//         ? [values.repositories[0].repository]
//         : [];
//       const pullRequests = values.pullRequests?.[0]?.pr
//         ? [values.pullRequests[0].pr]
//         : [];
...
//               loading={
//                 createReleaseNote.isPending || updateReleaseNote.isPending
//               }
//             >
//               Save Draft
//             </Button>
...
//                       </Col>
//                       <Col span={12}>
//                         {/* ✅ Version field with Last Version as separate label above */}
//                         {/* <div
//                           style={{
//                             marginBottom: 8
//                           }}
...
//                           {(fields) => (
//                             <Form.Item label="Pull Requests" name={[0, "pr"]}>
//                               <Select
//                                 placeholder="Select pull request"
//                                 style={{ width: "100%" }}
//                                 allowClear
//                               >
//                                 <Select.Option value="PR-45">
//                                   PR-45
//                                 </Select.Option>
//                                 <Select.Option value="PR-46">
//                                   PR-46
//                                 </Select.Option>
//                               </Select>
//                             </Form.Item>
...
//                           disabled={!selectedProjectId || !repositoryInput.trim()}
//                         >
//                           Add
//                         </Button>
//                       </Space.Compact>
...
//                           disabled={!selectedProjectId || !pullRequestInput.trim()}
//                         >
//                           Add
//                         </Button>
//                       </Space.Compact>
                      
//                       {/* Display added pull requests */}
...
//   );
// } version,enviroment,pr,repo all are working 
...
//   // ✅ Fetch environments from service - ONLY from API, no hardcoded fallback
//   // useEffect(() => {
//   //   const fetchEnvironments = async () => {
//   //     try {
//   //       setLoadingEnvironments(true);
...
//   //       console.log("Environments API response:", response); // Debug log
...
//   //       // If no environments were set but API returned successfully, show error
//   //       if (environments.length === 0) {
//   //         console.warn("No environments returned from API");
//   //       }
//   //     } catch (error) {
//   //       console.error("Failed to fetch environments:", error);
//   //       message.error("Failed to load environments");
...
//       // The response is directly the array of environments
//       if (Array.isArray(response)) {
//         setEnvironments(response);
//       } else {
...
//       const summaryContent = getDocumentArray(releaseData.summary);
//       if (summaryContent) summaryEditor.replaceBlocks(summaryEditor.document, summaryContent);

//       const keyInsightsContent = getDocumentArray(releaseData.keyInsights);
//       if (keyInsightsContent) keyInsightsEditor.replaceBlocks(keyInsightsEditor.document, keyInsightsContent);

//       const newFeaturesContent = getDocumentArray(releaseData.newFeatures);
//       if (newFeaturesContent) newFeaturesEditor.replaceBlocks(newFeaturesEditor.document, newFeaturesContent);

//       const improvementsContent = getDocumentArray(releaseData.improvements);
//       if (improvementsContent) improvementsEditor.replaceBlocks(improvementsEditor.document, improvementsContent);

//       const bugFixesContent = getDocumentArray(releaseData.bugFixes);
//       if (bugFixesContent) bugFixesEditor.replaceBlocks(bugFixesEditor.document, bugFixesContent);

//       const breakingChangesContent = getDocumentArray(releaseData.breakingChanges);
//       if (breakingChangesContent) breakingChangesEditor.replaceBlocks(breakingChangesEditor.document, breakingChangesContent);

//       const apiChangesContent = getDocumentArray(releaseData.apiChanges);
//       if (apiChangesContent) apiChangesEditor.replaceBlocks(apiChangesEditor.document, apiChangesContent);

//       const databaseChangesContent = getDocumentArray(releaseData.databaseChanges);
//       if (databaseChangesContent) databaseChangesEditor.replaceBlocks(databaseChangesEditor.document, databaseChangesContent);

//       const knownIssuesContent = getDocumentArray(releaseData.knownIssues);
//       if (knownIssuesContent) knownIssuesEditor.replaceBlocks(knownIssuesEditor.document, knownIssuesContent);
//     }
//   }, [existingRelease, isDuplicateMode]);
...
//         releaseDate: dayjs(),
//         visibility: [],
//         // Don't set default environment - let user select
//       });
//     }
//   }, [editId, duplicateId]);
...
//     if (activeEditor.title === "Release Summary") {
//       setActiveEditor((prev) => prev ? { ...prev, content: summaryEditor.document } : null);
//     } else if (activeEditor.title === "Key Insights") {
//       setActiveEditor((prev) => prev ? { ...prev, content: keyInsightsEditor.document } : null);
//     } else if (activeEditor.title === "New Features") {
//       setActiveEditor((prev) => prev ? { ...prev, content: newFeaturesEditor.document } : null);
//     } else if (activeEditor.title === "Improvements") {
//       setActiveEditor((prev) => prev ? { ...prev, content: improvementsEditor.document } : null);
//     } else if (activeEditor.title === "Bug Fixes") {
//       setActiveEditor((prev) => prev ? { ...prev, content: bugFixesEditor.document } : null);
//     } else if (activeEditor.title === "Breaking Changes") {
//       setActiveEditor((prev) => prev ? { ...prev, content: breakingChangesEditor.document } : null);
//     } else if (activeEditor.title === "API Changes") {
//       setActiveEditor((prev) => prev ? { ...prev, content: apiChangesEditor.document } : null);
//     } else if (activeEditor.title === "Database Changes") {
//       setActiveEditor((prev) => prev ? { ...prev, content: databaseChangesEditor.document } : null);
//     } else if (activeEditor.title === "Known Issues") {
//       setActiveEditor((prev) => prev ? { ...prev, content: knownIssuesEditor.document } : null);
//     }
//   }, [
//     summaryEditor.document,
//     keyInsightsEditor.document,
...
//                       </Col>
//                       <Col span={12}>
//                         <Form.Item label="Version" name="version" rules={[{ required: true, message: "Please enter version" }]}>
...
//                         <Form.List name="repositories" initialValue={[{}]}>
//                           {(fields) => (
//                             <Form.Item label="Repositories" name={[0, "repository"]}>
//                               <Select placeholder="Select repository" style={{ width: "100%" }} allowClear>
...
//                           {(fields) => (
//                             <Form.Item label="Pull Requests" name={[0, "pr"]}>
//                               <Select placeholder="Select pull request" style={{ width: "100%" }} allowClear>
...
//   // Create a ref to track if we're updating from the effect
...
//   // FIXED: Updated useEffect to prevent infinite loop
//   useEffect(() => {
//     if (!activeEditor) return;

//     // Don't update if we're already in an update cycle
//     if (isUpdatingFromEffect.current) return;

//     let newContent = null;
    
//     if (activeEditor.title === "Release Summary") {
//       newContent = summaryEditor.document;
//     } else if (activeEditor.title === "Key Insights") {
//       newContent = keyInsightsEditor.document;
//     } else if (activeEditor.title === "New Features") {
//       newContent = newFeaturesEditor.document;
//     } else if (activeEditor.title === "Improvements") {
//       newContent = improvementsEditor.document;
//     } else if (activeEditor.title === "Bug Fixes") {
//       newContent = bugFixesEditor.document;
//     } else if (activeEditor.title === "Breaking Changes") {
//       newContent = breakingChangesEditor.document;
//     } else if (activeEditor.title === "API Changes") {
//       newContent = apiChangesEditor.document;
//     } else if (activeEditor.title === "Database Changes") {
//       newContent = databaseChangesEditor.document;
//     } else if (activeEditor.title === "Known Issues") {
//       newContent = knownIssuesEditor.document;
//     }

//     // Only update if content has actually changed
//     if (newContent && JSON.stringify(newContent) !== JSON.stringify(activeEditor.content)) {
...
//     }
//   }, [
//     activeEditor?.title, // Only depend on title, not the entire activeEditor
//     summaryEditor.document,
//     keyInsightsEditor.document,
//     newFeaturesEditor.document,
//     improvementsEditor.document,
...
//   // FIXED: Update preview in real-time as user types
//   useEffect(() => {
//     if (!activeEditor) return;
