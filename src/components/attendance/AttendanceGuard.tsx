...
import { getAttendanceNavItem, canAccessAttendanceItem } from './navItems';

// Route-level permission gate for an Attendance sub-page. Pass the nav item key;
...
export default function AttendanceGuard({
  ...
  const item = getAttendanceNavItem(itemKey);

  const allowed = item ? canAccessAttendanceItem(perms, item) : false;
